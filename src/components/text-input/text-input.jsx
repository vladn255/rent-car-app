...
const TextInput = ({ value, setLocationDataValue, inputInfo: { name, additionalClass, placeholder, label } }) => {
    ...
    return (
        <>
            <input className={`${additionalClass}-form__input form__input`} name={name} id={name} value={value} placeholder={placeholder} onChange={inputChangeHandler} required ref={inputRef}></input>
